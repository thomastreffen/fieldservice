...
export function CategoryManager({ open, onOpenChange, tenantId }: Props) {
  ...
  const { data: categories = [] } = useQuery({
    queryKey: ["order-form-categories", tenantId],
  ...
  const { data: templateCounts = {} } = useQuery({
    queryKey: ["category-template-counts", tenantId],
    enabled: !!tenantId && open,
    queryFn: async () => {
      const { data } = await (supabase as any)
    ...
    <>
          ...
          <div className="space-y-3">
            {categories.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-4">
                Ingen kategorier opprettet ennu
              </p>
            ) : (
              <div className="space-y-1.5">
                {categories.map((cat: any, idx: number) => (
                  <div
                    key={cat.id}
                    className="flex items-center gap-2 p-2.5 rounded-lg bg-muted/40 group"
                  >
                    <div className="flex flex-col shrink-0">
                      <button
                        ...
                        disabled={idx === 0}
                        onClick={() =>
                          reorderMutation.mutate({ id: cat.id, direction: "up" })
                        }
                      >
            ...
            <div className="flex gap-2">
              <Input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && newName && addMutation.mutate()
                }
              />
              <Button
                size="sm"
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Slett kategori "{deleteTarget?.name}"?
            </AlertDialogTitle>
            <AlertDialogDescription>
              {templateCount(deleteTarget?.id) > 0 ? (
                <>
                  Denne kategorien har {templateCount(deleteTarget?.id)} skjema
                  knyttet til seg. Velg hva som skal skje med dem:
                </>
              ) : (
          ...
          {templateCount(deleteTarget?.id) > 0 && (
            <div className="space-y-3 py-2">
              <div className="flex items-center gap-3">
                ...
                />
                <label htmlFor="uncategorize" className="text-sm">
                  Fjern kategori fra skjemaene (ukategorisert)
                </label>
              </div>
              <div className="flex items-center gap-3">
                <input
