      ...
      if (target === "case") {
        ...
        const { data: newCase, error: caseErr } = await (supabase as any)
          ...
          .insert({
            ...
            status: "open",
            scope: "internal",
            next_action: "waiting_for_assignment",
        ...
        if (description) {
          await (supabase as any).from("case_items").insert({
            case_id: newCase.id,
            tenant_id: activeCompanyId,
            type: "note",
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Konvertere bestilling</SheetTitle>
          <SheetDescription>
        ...
        <div className="mt-6 space-y-6">
          <RadioGroup
            value={target}
            onValueChange={(v) => setTarget(v as "case" | "order")}
            className="space-y-3"
          >
            <div
              className={`flex items-center gap-3 p-4 rounded-lg border-2 cursor-pointer transition-colors ${
                target === "case" ? "border-primary bg-primary/5" : "border-border hover:bg-muted/30"
              }`}
            >
              <RadioGroupItem value="case" id="conv-case" />
              <Label htmlFor="conv-case" className="cursor-pointer flex-1">
                <span className="font-medium text-sm">Konvertere til sak</span>
                <p className="text-xs text-muted-foreground">Opprett en sak i Henvendelser for videre oppfølging</p>
              </Label>
            </div>
            <div
              className={`flex items-center gap-3 p-4 rounded-lg border-2 cursor-pointer transition-colors ${
                target === "order" ? "border-primary bg-primary/5" : "border-border hover:bg-muted/30"
              }`}
            >
              <RadioGroupItem value="order" id="conv-order" />
              <Label htmlFor="conv-order" className="cursor-pointer flex-1">
                <span className="font-medium text-sm">Konvertere til oppdrag</span>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              ...
              <Badge variant="outline">
                Ref: {values.referanse_po || values.midlertidlig_referanse || "Ikke angitt"}
              </Badge>
