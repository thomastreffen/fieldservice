  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      for (const s of sections) {
        for (const f of s.fields || []) {
          ...
          if (!valuesEqual(oldV, newV)) {
            changes.push({ key: f.field_key, label: f.label, oldValue: oldV, newValue: newV, fieldType: f.field_type });
          }
        }
      }
      ...
      if (user?.id) {
        const { data: ua } = await supabase
          ...
          .eq("user_id", user.id)
          .maybeSingle();
        userName = (ua as any)?.full_name || userName;
      }
      ...
      for (const c of changes) {
        ...
        if (c.newValue != null && c.newValue !== "") {
          await (supabase as any).from("order_form_submission_values").insert({
    ...
    onSuccess: (result) => {
      if (result.changes.length === 0) {
        ...
        toast.success(`${result.changes.length} felt oppdatert`);
      }
