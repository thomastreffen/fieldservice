...
function groupFieldsIntoRows(fields: any[]) {
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    const fraction =
      w === "half" ? 0.5 : w === "third" ? 1 / 3 : w === "two_thirds" ? 2 / 3 : 1;
...
function getWidthStyle(w: string): string {
  switch (w) {
    ...
    case "two_thirds":
      return "w-full sm:w-[calc(66.666%-4px)]";
    default:
      return "w-full";
  }
}
        ...
        return (
          <div className="flex gap-4">
            <div className="flex items-center gap-1.5">
              <RadioGroupItem value="ja" disabled className="h-3.5 w-3.5" />
              <span className="text-[11px]">Ja</span>
            </div>
            <div className="flex items-center gap-1.5">
              <RadioGroupItem value="nei" disabled className="h-3.5 w-3.5" />
              <span className="text-[11px]">Nei</span>
            </div>
          </div>
        );
      case "checkbox_list":
...
export function BuilderCanvas({
  ...
  const computeDropIndex = useCallback(
    (e: React.DragEvent, containerEl: HTMLElement, fields: any[]): number => {
  ...
  const handleDragOver = useCallback(
    (e: React.DragEvent, sectionId: string, fields: any[]) => {
      e.preventDefault();
      e.stopPropagation();
      ...
      <div className="max-w-4xl mx-auto pt-6 pb-12 px-6">
        <div className="text-center mb-6">
          <h2 className="text-lg font-bold text-foreground">
            {templateTitle || "Nytt skjema"}
          </h2>
          <p className="text-[10px] text-muted-foreground mt-0.5">
            {sections.filter((s) => s.is_active !== false).length} seksjoner ·{" "}
            {sections.reduce(
              (n: number, s: any) =>
                n + (s.fields?.filter((f: any) => f.is_active !== false)?.length || 0),
              0
            )}{" "}
            felt
          </p>
            ...
            return (
              <div
                ...
                className={`rounded-3xl border bg-card transition-all ${
                  isSelected
                    ? "border-primary ring-2 ring-primary/10 shadow-[var(--shadow-card)]"
              ...
              >
                <div className="flex items-center gap-2 px-6 py-4 border-b border-border/20 bg-muted/20 rounded-t-3xl">
                  ...
                  {section.description && (
                    <span className="text-[10px] text-muted-foreground truncate max-w-[140px] hidden lg:block">
                      {section.description}
                    </span>
                  )}
                  <Badge variant="outline" className="text-[10px] font-normal shrink-0">
                    {activeFields.length} felt
                  </Badge>
                  ...
                  >
                    {fields.length === 0 ? (
                      ...
                      <div className="space-y-4">
                              ...
                              {row.fields.map((field: any, fIdx: number) => {
                                ...
                                return (
                                  ...
                                  >
                                    <div
                                      ...
                                      onDragStart={(e) => {
                                        e.dataTransfer.setData(
                                          "move-field",
                                          JSON.stringify({ fieldId: field.id, fromSectionId: section.id })
                                        );
                                    ...
                                    >
                                      <div
                                        className={`absolute -top-2 -right-1 flex items-center gap-0.5 z-10 transition-opacity ${
                        ...
                        {dropTarget?.sectionId === section.id &&
                          dropTarget.index >= fields.length && (
