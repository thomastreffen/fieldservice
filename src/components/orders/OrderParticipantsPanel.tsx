...
export function OrderParticipantsPanel({ submissionId, tenantId }: Props) {
  const { user, tenantId: activeCompanyId } = useAuth();
  const qc = useQueryClient();
  const [addOpen, setAddOpen] = useState(false);
  const [addType, setAddType] = useState<"internal_user" | "external_email">(
    "internal_user"
  );
  ...
  const addParticipant = useMutation({
    mutationFn: async (p: {
      ...
      const existing = participants.find(
        (ep) =>
          (p.user_id && ep.user_id === p.user_id) ||
          (p.email && ep.email === p.email)
      );
    ...
    onError: (err: any) =>
      toast.error(err.message || "Kunne ikke legge til deltaker"),
  });
        ...
        {participants.map((p) => (
          <div key={p.id} className="flex items-center gap-2 py-1 group">
            ...
            <Badge variant="outline" className="text-[8px] shrink-0">
              {p.participant_type === "internal_user"
                ? "Intern"
          ...
          <PopoverContent align="start" className="w-80 p-3 space-y-3">
            <div className="flex gap-1">
              <Button
                size="sm"
                variant={
                  addType === "internal_user" ? "default" : "outline"
                }
              ...
              <Button
                size="sm"
                variant={
                  addType === "external_email" ? "default" : "outline"
                }
                className="text-xs h-7 flex-1"
              ...
              <>
                ...
                <Select value={extRole} onValueChange={setExtRole}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Prosjektleder">
                      Prosjektleder
                    </SelectItem>
