...
export default function OrderFormBuilderPage() {
  ...
  const updateField = useMutation({
    mutationFn: async ({ fieldId, updates }: { fieldId: string; updates: Record<string, any> }) => {
      const { error } = await (supabase as any).from("order_form_template_fields").update(updates).eq("id", fieldId);
      if (error) throw error;
    },
  ...
  const updateSection = useMutation({
    mutationFn: async ({ sectionId, updates }: { sectionId: string; updates: Record<string, any> }) => {
      const { error } = await (supabase as any).from("order_form_template_sections").update(updates).eq("id", sectionId);
      if (error) throw error;
    },
    onSuccess: invalidate,
  });
  ...
  const deleteField = useMutation({
    mutationFn: async (fieldId: string) => {
      const { error } = await (supabase as any).from("order_form_template_fields").delete().eq("id", fieldId);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const deleteSection = useMutation({
    mutationFn: async (sectionId: string) => {
      await (supabase as any).from("order_form_template_fields").delete().eq("section_id", sectionId);
      const { error } = await (supabase as any).from("order_form_template_sections").delete().eq("id", sectionId);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const addSection = useMutation({
    mutationFn: async (title: string) => {
      ...
      const { error } = await (supabase as any).from("order_form_template_sections").insert({
        template_id: id!,
        title,
  ...
  const addFieldToSection = useCallback(async (
    ...
    if (insertAtIndex !== undefined && insertAtIndex < sectionFields.length) {
      ...
      for (const f of bumps) {
        await (supabase as any).from("order_form_template_fields").update({ sort_order: f.sort_order + 1 }).eq("id", f.id);
      }
    } else {
    ...
    const { error } = await (supabase as any).from("order_form_template_fields").insert({
      template_id: id!,
  ...
  const addBlockToSection = useCallback(async (block: FieldBlock, sectionId: string, insertAtIndex?: number) => {
    ...
    const { error } = await (supabase as any).from("order_form_template_fields").insert(rows);
    if (error) { toast.error(error.message); return; }
    invalidate();
  ...
  const moveSection = useCallback(async (fromIdx: number, toIdx: number) => {
    ...
    const updates = sorted.map((s: any, i: number) =>
      (supabase as any).from("order_form_template_sections").update({ sort_order: i }).eq("id", s.id)
    );
  ...
  const moveField = useCallback(async (fieldId: string, fromSectionId: string, toSectionId: string, toIndex: number) => {
    ...
    if (fromSectionId !== toSectionId) {
      await (supabase as any).from("order_form_template_fields").update({ section_id: toSectionId }).eq("id", fieldId);
    }

    // Re-number all fields in the target section
    const updates = toFields.map((f: any, i: number) =>
      (supabase as any).from("order_form_template_fields").update({ sort_order: i }).eq("id", f.id)
    );
    await Promise.all(updates);
  ...
  const updateTemplate = useMutation({
    mutationFn: async (updates: Record<string, any>) => {
      const { error } = await (supabase as any).from("order_form_templates").update(updates).eq("id", id!);
      if (error) throw error;
    },
    onSuccess: () => {
...
function TemplateSettingsForm({ template, onSave }: { template: any; onSave: (u: any) => void }) {
  const { tenantId: activeCompanyId } = useAuth();
