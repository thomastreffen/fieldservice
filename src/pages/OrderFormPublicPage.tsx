...
export default function OrderFormPublicPage() {
  ...
  const { data: template, isLoading, error: loadError } = useQuery({
    ...
    enabled: !!slug,
    queryFn: async () => {
      // Find active template by slug (no tenant filter for public)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const tenantId = (template as any).tenant_id;
      const { error: subErr } = await (supabase as any).from("order_form_submissions").insert({
        id: submissionId,
        tenant_id: tenantId,
        ...
        public_tracking_token: newTrackingToken,
      });
      if (subErr) throw subErr;

      // Normalize values to JSON-safe form
      ...
      if (valueRows.length > 0) {
        const { error: valuesErr } = await (supabase as any)
        ...
        if (valuesErr) {
          await (supabase as any).from("order_form_activity_log").insert({
            ...
            payload: {
              ...
              field_keys: valueRows.map((r: any) => r.field_key),
            },
          });
      ...
      for (const att of attachments) {
        const path = `${tenantId}/${submissionId}/${Date.now()}_${att.file.name}`;
        ...
        const { error: attInsErr } = await (supabase as any).from("order_form_submission_attachments").insert({
