...
import mascotReceived from "@/assets/mascot/received.svg";
import mascotProcessing from "@/assets/mascot/processing.svg";
import mascotPlanned from "@/assets/mascot/planned.svg";
import mascotInProgress from "@/assets/mascot/in_progress.svg";
import mascotCompleted from "@/assets/mascot/completed.svg";
...
export default function OrderTrackingPage() {
  ...
  useEffect(() => {
    if (submission?.id) {
      (supabase as any).from("order_form_submissions")
  ...
  const submitReply = useMutation({
    mutationFn: async () => {
      ...
      for (const file of replyFiles) {
        const path = `${sub.tenant_id || sub.company_id}/${submission.id}/reply_${Date.now()}_${file.name}`;
        await supabase.storage.from("order-form-attachments").upload(path, file);
        await (supabase as any).from("order_form_submission_attachments").insert({
                ...
                <div className="space-y-3 pt-4 border-t border-border/60">
                  {isInternalViewer && (
                    <div className="rounded-lg border border-amber-300/60 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800/60 px-3 py-2 text-xs text-amber-900 dark:text-amber-200 flex items-start gap-2">
                      ...
                      <span>
                        Du er innlogget som <strong>{user?.email}</strong>. Meldingen sendes som <strong>MCS Service</strong>, ikke som kunde.
                      </span>
                    </div>
                  )}
                  <Textarea
