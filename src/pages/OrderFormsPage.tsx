...
export default function OrderFormsPage() {
  ...
  const { user, tenantId: activeCompanyId, isTenantAdmin, isMasterAdmin } = useAuth();
      ...
      if (assigneeIds.length > 0) {
        ...
        if (profiles) {
          (profiles as any[]).forEach(p => {
            if (p.full_name) assigneeMap.set(p.user_id, p.full_name);
          });
        }
      }
  ...
  const handleSoftDelete = async (e: React.MouseEvent, sub: any) => {
    ...
    await (supabase as any).from("order_form_submissions").update({
