...
export default function OrderFormDetailPage() {
  ...
  // Fetch available users for assignment from profiles
  ...
  const addComment = useMutation({
    mutationFn: async () => {
      ...
      const { data: ua } = await supabase
        ...
        .eq("is_active", true)
        .maybeSingle();
      const senderName = (ua as any)?.full_name || "Saksbehandler";
      ...
      const { data: insertedMsg, error } = await (supabase as any).from("order_form_messages").insert({
        submission_id: id!,
        sender_type: "admin",
      ...
      await (supabase as any).from("order_form_comments").insert({
        submission_id: id!,
        body: comment.trim(),
      ...
      if (isShared) {
        await (supabase as any).from("order_form_submissions")
          .update({ last_admin_message_at: new Date().toISOString(), last_activity_at: new Date().toISOString() })
          .eq("id", id!);
      }
  ...
  const assignResponsible = useMutation({
    mutationFn: async (assigneeId: string | null) => {
      ...
      // Log activity
      const assigneeNameStr = assignee?.name || "Ingen";
      await (supabase as any).from("order_form_activity_log").insert({
        submission_id: id!,
        event_type: "assigned",
        payload: {
          assigned_to: assigneeId,
          assigned_to_name: assigneeNameStr,
        },
        created_by: user?.id,
      });
      ...
      if (assigneeId && assigneeId !== user?.id) {
        await (supabase as any).from("notifications").insert({
          user_id: assigneeId,
          tenant_id: (submission as any)?.tenant_id || activeCompanyId,
          type: "order_assigned",
          priority: "important",
  ...
  return (
    <div className="space-y-5 p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
                ...
                <label className="flex items-center gap-2 cursor-pointer select-none group">
                  <Checkbox
                    ...
                    onCheckedChange={async (checked) => {
                      await (supabase as any).from("order_form_submissions")
                        .update({ auto_notify_on_status_change: !!checked })
                        .eq("id", id!);
                      qc.invalidateQueries({ queryKey: ["order-form-submission", id] });
          ...
          <OrderParticipantsPanel
            submissionId={id!}
            tenantId={(submission as any).tenant_id || activeCompanyId}
          />
                        ...
                        {isRequestInfo && m.replied_at && !m.reviewed_at && (
                          <div className="flex items-center gap-2 mt-2 pt-2 border-t border-border/50">
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                await (supabase as any).from("order_form_messages")
                                  .update({
      ...
      <LinkExistingTaskDialog
        ...
        submissionId={id!}
        submissionNo={submission.submission_no}
        submissionCompanyId={(sub as any).tenant_id || activeCompanyId}
