...
function HmsOvertimePageInner() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["hms-overtime", tenantId, tab],
    enabled: !!tenantId,
    queryFn: async () => {
      ...
      if (userIds.length) {
        const { data: u } = await sb.from("profiles").select("user_id, full_name, email").in("user_id", userIds);
        names = Object.fromEntries((u ?? []).map((x: any) => [x.user_id, x.full_name || x.email || "Ukjent"]));
      }
    ...
    mutationFn: async () => {
      ...
      try {
        await (supabase as any).functions.invoke("worktime-aml-evaluate", {
          body: { tenant_id: tenantId!, user_id: active.user_id },
        });
      } catch {}
    },
    onSuccess: () => {
      toast(decision === "approve" ? "Overtid godkjent" : "Overtid avvist");
