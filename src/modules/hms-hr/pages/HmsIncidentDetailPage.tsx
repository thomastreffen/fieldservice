...
export default function HmsIncidentDetailPage() {
  ...
  const namesQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await sb
        ...
        .in("user_id", userIds);
      return Object.fromEntries((data ?? []).map((p: any) => [p.user_id, p.full_name || p.email || "Ukjent"])) as Record<string, string>;
    },
  });
  const names = namesQuery.data ?? {};

  const usersQuery = useQuery({
    queryKey: ["hms-assignable-users", tenantId],
    ...
    queryFn: async () => {
      ...
      const out: { id: string; name: string }[] = (data ?? []).map((p: any) => ({
        id: p.user_id,
  ...
  const updateIncident = useMutation({
    mutationFn: async (patch: Record<string, any>) => {
      ...
      if (error) throw error;
      await logHmsAudit({
        tenant_id: tenantId!, action: "incident.updated",
  ...
  const addComment = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await sb.from("hms_incident_comments").insert({
        incident_id: id, tenant_id: i.tenant_id, author_id: user!.id, body: comment.trim(),
      });
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const createAction = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await sb.from("hms_action_items").insert({
        tenant_id: i.tenant_id, title: actionTitle.trim(),
