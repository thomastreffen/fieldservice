...
function HmsWorktimeImportPageInner() {
  const navigate = useNavigate();
  const qc = useQueryClient();
  const { tenantId, employeeProfiles } = useHmsContextReady();
  ...
  // Derive people list from context's employeeProfiles
  const people = useMemo(
    () => employeeProfiles.map((p) => ({ id: p.user_id, full_name: p.full_name ?? null, email: p.email ?? null })),
    [employeeProfiles],
  );

  // Load employee_work_profiles for external_employee_id matching
  const { data: extProfiles } = useQuery({
    queryKey: ["employee-ext-profiles", tenantId],
    enabled: !!tenantId,
    queryFn: async () => {
  ...
  }, [monthly, extProfiles, people, manualMap]);
  ...
  async function handleFile(f: File) {
    ...
    try {
      ...
      if (looksLikeMonthlyOverview(hdrs)) {
        ...
        setRawRows(rows);
        setStep("match");
        toast("Tripletex månedsoversikt detektert", {
      ...
      toast.error("Feil ved lesing", { description: String(e.message || e) });
    }
  }

  const saveMappingMut = useMutation({
    mutationFn: async (entries: { user_id: string; external_employee_id: string }[]) => {
      if (!tenantId) throw new Error("Mangler tenant");
      ...
      await qc.invalidateQueries({ queryKey: ["employee-ext-profiles", tenantId] });
    },
  });
  ...
  const importMut = useMutation({
    mutationFn: async () => {
      ...
      for (const m of matched) {
        ...
        const hash = await buildSourceHash([
          tenantId!, m.user_id, r.work_date, r.start_at, r.end_at,
        ...
        if (!existing) {
          const { data } = await sb.from("worktime_entries").select("id")
            .eq("tenant_id", tenantId!).eq("source_hash", hash).maybeSingle();
          existing = data;
        }
        if (existing) {
      ...
      const { data: aml, error: amlErr } = await sb.functions.invoke("worktime-aml-evaluate", {
        body: { tenant_id: tenantId!, batch_id: batch.id },
      });
    ...
    onSuccess: async (r) => {
      ...
      await qc.invalidateQueries({ predicate: (q) => {
        ...
        return typeof k === "string" && (k.startsWith("hms-aml") || k === "employee-ext-profiles" || k === "hms-import-batches");
      }});
      toast("Import fullført", { description: `${r.inserted} nye, ${r.updated} oppdatert` });
    },
    onError: (e: any) => toast.error("Import feilet", { description: String(e.message || e) }),
  });

  const importMonthlyMut = useMutation({
    mutationFn: async () => {
      if (!tenantId || !monthly) throw new Error("Mangler tenant eller fil");
      const sb = supabase as any;
      const { data: batch, error: bErr } = await sb
        .from("worktime_import_batches")
        .insert({
          ...
          <CardContent className="space-y-3">
            {people.length === 0 ? (
              <div className="p-4 rounded-md bg-amber-500/10 border border-amber-500/30 text-sm">
                Ingen ansatte funnet. Sjekk bruker-/ansattregister eller tilgang.
              </div>
            ) : unmatchedEmpCount > 0 && (
              <div className="p-3 rounded-md bg-amber-500/10 border border-amber-500/30 text-xs">
                <strong>{unmatchedEmpCount} ansatte mangler kobling.</strong> Velg riktig ansatt fra listen. Tripletex ansattnummer lagres på ansatten for fremtidige importer.
              </div>
            )}
            <div className="overflow-x-auto">
              <Table>
                        ...
                        <TableCell>
                          ...
                          >
                            ...
                            <SelectContent>
                              ...
                              {people.length === 0 && (
                                <div className="px-2 py-2 text-xs text-muted-foreground">Ingen ansatte funnet</div>
                              )}
                              {people.map((p) => {
              ...
              <Button
                onClick={async () => {
                  ...
                  try {
                    ...
                    toast.error("Kunne ikke lagre kobling", { description: String(err.message || err) });
                  }
                  setStep("preview");
                }}
