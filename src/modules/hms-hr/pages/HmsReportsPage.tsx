...
function downloadCsv(filename: string, rows: Record<string, any>[]) {
  if (!rows.length) {
    toast.error("Ingen data", { description: "Ingen rader å eksportere" });
    return;
  }
...
function HmsReportsPageInner() {
  ...
  async function lookupNames(userIds: string[]) {
    ...
    return Object.fromEntries(
      (data ?? []).map((x: any) => [x.user_id, x.full_name || x.email || x.user_id])
    );
  }
  ...
  async function exportAml(status: "open" | "all") {
    ...
    try {
      ...
      let q = sb.from("worktime_alerts").select("*").eq("tenant_id", tenantId!)
  ...
  async function exportOvertime() {
    ...
    try {
      ...
      const { data } = await sb.from("overtime_approvals").select("*")
        .eq("tenant_id", tenantId!).gte("period_end", from).lte("period_end", to);
  ...
  async function exportSubmissions() {
    ...
    try {
      ...
      const { data } = await sb.from("hms_submissions").select("*")
        .eq("tenant_id", tenantId!).gte("submitted_at", from).lte("submitted_at", to + "T23:59:59")
        .is("deleted_at", null);
  ...
  async function exportHandbookAcks() {
    ...
    try {
      ...
      const { data } = await sb.from("hms_handbook_acknowledgements").select("*, hms_handbooks(title)")
        .eq("tenant_id", tenantId!).gte("acknowledged_at", from).lte("acknowledged_at", to + "T23:59:59");
      const names = await lookupNames(Array.from(new Set((data ?? []).map((a: any) => a.user_id))));
