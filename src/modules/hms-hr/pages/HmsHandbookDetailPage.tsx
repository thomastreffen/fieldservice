...
interface Handbook {
  ...
  current_version_id: string | null; updated_at: string;
}
interface Version {
  id: string; handbook_id: string; version_number: number; status: string;
...
function HmsHandbookDetailPageInner() {
  const { id } = useParams<{ id: string }>();
  const { tenantId, canManageHms } = useHmsContextReady();
  const { user } = useAuth();
  const canManage = canManageHms;
  ...
  const { data: handbook, isLoading } = useQuery({
    queryKey: ["hms-handbook", id, tenantId],
    enabled: !!id && !!tenantId,
    queryFn: async () => {
      const sb = supabase as any;
      const { data, error } = await sb
        .from("hms_handbooks")
        .select("id, title, description, kind, status, current_version_id, updated_at")
  ...
  const { data: versions = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await sb
        ...
        .select("id, handbook_id, version_number, status, requires_acknowledgement, published_at, published_by, changelog, created_at")
        .eq("handbook_id", id)
  ...
  const { data: ackOverview } = useQuery({
    queryKey: ["hms-handbook-ack-overview", publishedVersion?.id, tenantId],
    enabled: !!publishedVersion && !!tenantId && canManage,
    queryFn: async () => {
      const sb = supabase as any;
      const [{ data: ewps }, { data: acks }] = await Promise.all([
        sb.from("employee_work_profiles")
          .select("user_id")
      ...
      const userIds: string[] = (ewps ?? []).map((e: any) => e.user_id);
      ...
      if (userIds.length) {
        ...
        for (const p of profs ?? []) nameMap[p.user_id] = { full_name: p.full_name, email: p.email };
      }
      const ackMap = new Map<string, string>((acks ?? []).map((a: any) => [a.user_id, a.acknowledged_at]));
      return userIds.map((uid) => ({
        user_id: uid,
        full_name: nameMap[uid]?.full_name ?? null,
        email: nameMap[uid]?.email ?? null,
        acknowledged_at: ackMap.get(uid) ?? null,
      }));
    },
  });
  ...
  const saveSection = useMutation({
    mutationFn: async (s: { id: string; heading: string; body: string }) => {
      ...
      if (error) throw error;
      await logHmsAudit({
        tenant_id: tenantId!, entity_type: "hms_handbook", entity_id: handbook?.id,
    ...
    onError: (e: any) => toast.error("Feil", { description: String(e.message || e) }),
  });
  ...
  const ackMut = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await sb.from("hms_handbook_acknowledgements").insert({
        handbook_id: handbook.id, version_id: publishedVersion.id, tenant_id: tenantId!,
  ...
  const aiDraftMut = useMutation({
    mutationFn: async (_mode: "draft" | "simplify" | "leader" | "short" | "checklist") => {
      console.warn("TODO: HMS AI generation edge function not yet deployed");
      throw new Error("AI-generering er ikke tilgjengelig ennå");
    },
    onError: () => toast.error("AI-generering er ikke tilgjengelig ennå"),
  });
