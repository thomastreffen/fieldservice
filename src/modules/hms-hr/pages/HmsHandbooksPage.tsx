  ...
  const seedMut = useMutation({
    mutationFn: async () => {
      ...
      for (const seed of MCS_HANDBOOK_SEEDS) {
        ...
        const sections = seed.chapters.map((c: any, i: number) => ({
          version_id: ver.id,
          heading: c.title,
          body: c.body,
          ordering: i,
        ...
        await logHmsAudit({
          tenant_id: tenantId!, entity_type: "hms_handbook", entity_id: hb.id,
          action: "handbook.created", payload: { kind: seed.kind, title: seed.title, source: "seed" },
        });
        created++;
      }
    ...
    onSuccess: (r) => {
      if (r.created === 0) {
        toast("Startstruktur finnes allerede", { description: `${r.skipped} håndbøker var allerede opprettet.` });
      } else {
        toast("Håndbøker opprettet", { description: `${r.created} ny${r.created === 1 ? "" : "e"}, ${r.skipped} hoppet over.` });
      }
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2">
          <Button size="sm" variant="outline" onClick={() => seedMut.mutate()} disabled={seedMut.isPending}>
            ...
            {seedMut.isPending ? "Oppretter…" : "Opprett startstruktur"}
          </Button>
        ...
        <Card className="border-dashed">
          <CardContent className="py-10 text-center text-sm text-muted-foreground space-y-3">
            ...
            <p className="max-w-sm mx-auto">
              Opprett startstruktur for å legge til HMS-håndbok, arbeidshåndbok og bedriftsprosedyrer
              med versjonering og lesebekreftelse.
            </p>
          </CardContent>
        </Card>
      ) : (
