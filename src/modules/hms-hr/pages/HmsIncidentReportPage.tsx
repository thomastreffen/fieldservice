...
export default function HmsIncidentReportPage() {
  ...
  const { data: projects } = useQuery({
    queryKey: ["hms-incident-projects", tenantId],
    enabled: !!tenantId,
    queryFn: async () => {
    ...
    >
      ...
      <div className="px-4 py-4 max-w-2xl mx-auto space-y-5">
            ...
            >
              ...
              {projects.map((p: any) => (
                <option key={p.id} value={p.id}>
                  {p.project_number ? `${p.project_number} – ` : ""}{p.title ?? p.name}
                </option>
              ))}
            </select>
          </div>
        ...
        <div className="text-[11px] text-muted-foreground bg-muted/30 rounded p-2">
          Rapportert av <span className="font-medium text-foreground">{(user as any)?.name || user?.email}</span> ·{" "}
