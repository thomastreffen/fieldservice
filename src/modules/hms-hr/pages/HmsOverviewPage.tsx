...
import {
  ...
  FileCheck, Inbox, FileBarChart2, ArrowRight,
  Smartphone, ShieldAlert,
} from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { useHmsContextReady, HmsContextGate } from "@/modules/hms-hr/context/HmsContext";
...
function HmsOverviewPageInner() {
  const { tenantId, canManageHms } = useHmsContextReady();
  const isMobile = useIsMobile();

  if (isMobile && !canManageHms) {
    return <Navigate to="/hms/mobile" replace />;
  }

  const { data, isLoading } = useQuery({
    queryKey: ["hms-overview-v2", tenantId],
    enabled: !!tenantId,
    queryFn: async () => {
      const cid = tenantId!;
      ...
      const [
        ...
        profiles, pendingReview, submitted7d, importBatchesIssues,
        openIncidents, criticalIncidents, overdueActions, unassignedIncidents,
      ] = await Promise.all([
        countOf(sb.from("hms_handbooks").select("id", { count: "exact", head: true }).eq("tenant_id", cid).is("deleted_at", null)),
        countOf(sb.from("worktime_alerts").select("id", { count: "exact", head: true }).eq("tenant_id", cid).eq("status", "open")),
        countOf(sb.from("worktime_alerts").select("id", { count: "exact", head: true }).eq("tenant_id", cid).eq("status", "open").eq("severity", "critical")),
        countOf(sb.from("overtime_approvals").select("id", { count: "exact", head: true }).eq("tenant_id", cid).eq("status", "pending")),
        countOf(sb.from("hms_action_items").select("id", { count: "exact", head: true }).eq("tenant_id", cid).in("status", ["open", "in_progress"]).is("deleted_at", null)),
        countOf(sb.from("employee_work_profiles").select("id", { count: "exact", head: true }).eq("tenant_id", cid)),
        countOf(sb.from("hms_submissions").select("id", { count: "exact", head: true }).eq("tenant_id", cid).eq("status", "submitted").is("deleted_at", null)),
        countOf(sb.from("hms_submissions").select("id", { count: "exact", head: true }).eq("tenant_id", cid).gte("submitted_at", since7).is("deleted_at", null)),
        countOf(sb.from("worktime_import_batches").select("id", { count: "exact", head: true }).eq("tenant_id", cid).gt("skipped_rows", 0)),
        countOf(sb.from("hms_incidents").select("id", { count: "exact", head: true }).eq("tenant_id", cid).is("deleted_at", null).not("status", "in", "(closed,rejected)")),
        countOf(sb.from("hms_incidents").select("id", { count: "exact", head: true }).eq("tenant_id", cid).is("deleted_at", null).in("severity", ["critical","high"]).not("status", "in", "(closed,rejected)")),
        countOf(sb.from("hms_action_items").select("id", { count: "exact", head: true }).eq("tenant_id", cid).in("status", ["open","in_progress"]).is("deleted_at", null).not("due_date", "is", null).lte("due_date", new Date().toISOString().slice(0,10))),
        countOf(sb.from("hms_incidents").select("id", { count: "exact", head: true }).eq("tenant_id", cid).is("deleted_at", null).is("assigned_to", null).not("status", "in", "(closed,rejected)")),
      ]);
      ...
      const { data: critAlerts } = await sb
        ...
        .eq("tenant_id", cid).eq("status", "open").eq("severity", "critical")
      ...
      const { data: penOt } = await sb
        ...
        .eq("tenant_id", cid).eq("status", "pending")
      ...
      const { data: pendSubs } = await sb
        ...
        .eq("tenant_id", cid).eq("status", "submitted").is("deleted_at", null)
      ...
      const { data: rejSubs } = await sb
        ...
        .eq("tenant_id", cid).eq("status", "rejected").is("deleted_at", null)
      ...
      const { data: dueActions } = await sb
        ...
        .eq("tenant_id", cid).in("status", ["open", "in_progress"]).is("deleted_at", null)
      ...
      if (topUserIds.length > 0) {
        const { data: profs } = await sb
          ...
          .in("user_id", topUserIds.map(([uid]) => uid));
        topNames = Object.fromEntries(
          (profs ?? []).map((a: any) => [a.user_id, a.full_name || a.email || "Ukjent"])
        );
      }
      ...
      const { data: critIncidents } = await sb
        ...
        .eq("tenant_id", cid).is("deleted_at", null)
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground max-w-2xl">
            Status på håndbøker, SJA, risiko og arbeidsmiljølovens grenser.
          </p>
        </div>
