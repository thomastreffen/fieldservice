...
function HmsImportBatchesPageInner() {
  const nav = useNavigate();
  const qc = useQueryClient();
  const { tenantId } = useHmsContextReady();

  const { data, isLoading } = useQuery({
    queryKey: ["hms-import-batches", tenantId],
    enabled: !!tenantId,
    queryFn: async () => {
  ...
  const rerunMut = useMutation({
    mutationFn: async (batchId: string) => {
      await (supabase as any).functions.invoke("worktime-aml-evaluate", {
        body: { tenant_id: tenantId!, batch_id: batchId },
      });
    },
    onSuccess: async () => {
      ...
      toast("AML-kontroll kjørt på nytt");
    },
